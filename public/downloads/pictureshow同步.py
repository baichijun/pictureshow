...
def load_config():
    ...
    # Token 是必填项，没有就交互式获取
    if not cfg["token"]:
        prompt_for_token(cfg)
    return cfg


def prompt_for_token(cfg):
    """引导用户粘贴具备写入权限的 Token 并保存。"""
    print_token_setup_guide(cfg["owner"], cfg["repo"])
    try:
...
def print_token_setup_guide(owner, repo):
    print("需要一个对仓库 %s/%s 有【写入】权限的 GitHub Token：" % (owner, repo))
    print("  方式 A（推荐，细粒度）: https://github.com/settings/personal-access-tokens/new")
    print("    - Repository access: 只选 %s" % repo)
    print("    - Repository permissions → Contents: 选 Read and write（不是 Read-only）")
    print("  方式 B（经典）: https://github.com/settings/tokens/new")
    print("    - 勾选 repo（完整仓库读写权限）")


def print_token_fix_403():
    print("\n【403 原因】当前 Token 没有向仓库写入文件的权限（多为 Contents 仍是 Read-only）。")
    print("请按下面步骤重新生成 Token，然后重新运行本工具并粘贴新 Token：")
    print("  1. 打开 https://github.com/settings/personal-access-tokens/new")
    print("  2. Repository access → Only select repositories → 勾选 pictureshow")
    print("  3. 找到 Contents → 下拉改为 Read and write（这一步最关键）")
    print("  4. Generate token → 复制新 token")
    print("  5. 删除旧配置后重试: %s" % CONFIG_PATH)
    print("     或在下次运行时输入新 token 覆盖保存。")


def verify_write_permission(cfg):
    """
    上传前检查 Token 是否具备 push/写入权限。
    返回 (ok: bool, message: str)，message 为 token_no_write 表示缺写入权限。
    """
    path = "/repos/%s/%s" % (cfg["owner"], cfg["repo"])
    status, data = api_request("GET", path, cfg["token"])
    if status != 200:
        msg = data.get("message") if isinstance(data, dict) else str(data)
        return False, "无法访问仓库 (HTTP %s): %s" % (status, msg)
    perms = (data or {}).get("permissions") or {}
    if perms.get("push"):
        return True, ""
    return False, "token_no_write"
...
def auth_header(token):
    """细粒度 PAT 推荐使用 Bearer；经典 PAT 两种均可。"""
    return "Bearer %s" % token.strip()
...
def upload_image(cfg, album_name, file_path, file_name):
    ...
    msg = data.get("message") if isinstance(data, dict) else str(data)
    if status == 403:
        raise RuntimeError("上传失败 (HTTP 403): Token 无写入权限，请按上文说明重新生成 Token")
...
def main():
    ...
    # 2b. 上传前检查写入权限，避免一张张失败
    ok_perm, perm_msg = verify_write_permission(cfg)
    if not ok_perm:
        if perm_msg == "token_no_write":
            print_token_fix_403()
            try:
                ans = input("\n是否现在输入新的 Token？(y/N): ").strip().lower()
            except EOFError:
                ans = ""
            if ans in ("y", "yes"):
                cfg["token"] = ""
                ...
                ok_perm, perm_msg = verify_write_permission(cfg)
                if not ok_perm:
                    print("\n新 Token 仍无写入权限，请确认 Contents 已设为 Read and write。")
                    return
            else:
                return
        else:
